...
from utils.metric import moses_multi_bleu, calculate_distinct_n
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x, encoder_outputs, emotion_context, attention_weight, mask = inputs
        ...
        y, attention_weight = self.multi_head_attention_enc_dec(x_norm, encoder_outputs, encoder_outputs, 
                                                                mask_src, emotion_context=emotion_context)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, mask, emotion_context=None):
        ...
        if emotion_context != None:
            emotion_context = emotion_context.unsqueeze(1).repeat(1, outputs.size(1), 1)
            outputs = torch.cat((outputs, emotion_context), dim=2)
...
def gen_embeddings(vocab, type_):
    """
    ...
    if type_ != 'vad':
        ...
        embeddings = np.ones((vocab.n_words, 3)) * 0.5
        embeddings[:, 1] = 0
    emb_file = config.emb_file if type_ == 'glove' else config.emb_file_senti if type_ == 'senti' else config.dir_vad
    ...
    if emb_file is not None:
        ...
        for line in open(emb_file).readlines():
            sp = line.split() if type_ == 'glove' or type_ == 'senti' else line.split('\t')
            #if(len(sp) == config.emb_dim + 1):
            if sp[0] in vocab.word2index:
                pre_trained += 1
                '''if type_ == 'vad':
                    embeddings[vocab.word2index[sp[0]]] = rescale_embeddings(np.array([float(x) for x in sp[1:]]))
                else: '''
                embeddings[vocab.word2index[sp[0]]] = [float(x) for x in sp[1:]]
            #else:
            #    print('pre-embedding size dif to used embedding size: ', sp[0])
...
def rescale_embeddings(embeddings):
    ...
    # Formula for linear transformation: y = (x - min_val) * (new_max - new_min) / (max_val - min_val) + new_min
    rescaled_embeddings = (embeddings - min_val) * (new_max - new_min) / (max_val - min_val) + new_min

    return rescaled_embeddings



class Embeddings(nn.Module):
    def __init__(self, vocab, type_='glove', padding_idx=None):
        super(Embeddings, self).__init__()
        self.type_ = type_
        d_model = config.emb_dim if type_ != 'vad' else 3
        self.lut = nn.Embedding(vocab, d_model, padding_idx=padding_idx)
        self.d_model = d_model

    def forward(self, x):
        return (self.lut(x) * math.sqrt(self.d_model)) if self.type_ != 'vad' else self.lut(x)

def share_embedding(vocab, pretrain=True, type_='glove'):
    embedding = Embeddings(vocab.n_words, type_, padding_idx=config.PAD_idx)
    if(pretrain):
        pre_embedding = gen_embeddings(vocab, type_)
        embedding.lut.weight.data.copy_(torch.FloatTensor(pre_embedding))
        embedding.lut.weight.data.requires_grad = True if type_ != 'vad' else False
    return embedding


def read_vad_file():
    dict_data = {}
    df = pd.read_csv(config.dir_vad, 
                keep_default_na=False, header=None, sep='\t')
    for index, row in df.iterrows(): #V, A, D
        dict_data[str(row[0]).lower()] = torch.FloatTensor([float(row[1]), float(row[2]), float(row[3])])
    print('size VAD: ', len(dict_data))

    return dict_data

def init_emo_vad_val(emotions_dict):
    dict_data = read_vad_file()
    vad_values = np.zeros((len(emotions_dict), 3))
    for idx, emotion in emotions_dict.items():
        try:
            vad_values[idx] = dict_data[emotion]
        except:
            if emotion == 'anticipating':
                vad_values[idx] = dict_data['anticipate']

    return torch.FloatTensor(vad_values)

def get_vad(dict_vad, arr_emo):
    val_vector = []
    for emo in arr_emo:
        val_vector.append(dict_vad[emo])

    return torch.stack(val_vector, dim=0).to(config.device)
...
class DynamicLabelSmoothing(nn.Module):
    def __init__(self, size, padding_idx, smoothing_start=0.1, smoothing_end=0.0, num_steps=10000):
    ...
    def get_smoothing(self):
        # Linearly interpolate between smoothing_start and smoothing_end
        return max(self.smoothing_end, self.smoothing_start - (self.step / self.num_steps) * (self.smoothing_start - self.smoothing_end))
    ...
    def forward(self, x, target):
        ...
        true_dist = x.data.clone()
        true_dist.fill_(smoothing / (self.size - 2))
...
def write_custum(emotion, dial, ref, hyp_g):
    ret = ""
    ret += "emotion:{}".format(emotion) + '\n'
...
def evaluate_generator(model, data,  ty='valid', max_dec_step=30, write_summary=False):
    ...
    for j, batch in pbar:
        loss, ppl, bce_prog, acc_prog, dir_loss = model.train_one_batch(batch, 0, train=False)

        l.append(loss)
        p.append(ppl)
        bce.append(bce_prog)
        acc.append(acc_prog)
        vad.append(dir_loss['vad_loss'])
        if(ty =="test"): 
            sent_g = model.decoder_greedy(batch, max_dec_step=max_dec_step)
            results_sent.append(sent_g)
            for i, (greedy_sent)  in enumerate(sent_g):
                rf = " ".join(batch["target_txt"][i])
                hyp_g.append(greedy_sent)
                ref.append(rf)
                temp = write_custum(emotion= batch["program_txt"][i],
                            dial=[" ".join(s) for s in batch['input_txt'][i]] if config.dataset=="empathetic" \
                                    else " ".join(batch['input_txt'][i]),
                            ref=rf,
                            hyp_g=greedy_sent)
    ...
    if ty == 'test':
        ...
        distinct_1, distinct_2 = calculate_distinct_n(results_sent)
        print("EVAL\tLoss\tPPL\tAccuracy\tBleu_g\tDistinct-1\tDistinct-2")
        print("{}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.2f}\t{:.2f}\t{:.2f}".format(
                ty,loss,math.exp(loss), acc, bleu_score_g, distinct_1, distinct_2))

        if write_summary:
            return loss, math.exp(loss), bce, acc, bleu_score_g, distinct_1, distinct_2, inf_results
        else:
            return loss, math.exp(loss), bce, acc, bleu_score_g, distinct_1, distinct_2
    else:
        print("EVAL\tLoss\tPPL\tAccuracy")
        print("{}\t{:.4f}\t{:.4f}\t{:.4f}".format(ty,loss,math.exp(loss), acc))
        if write_summary:
            return loss, math.exp(loss), bce, acc, inf_results
        else:
            return loss, math.exp(loss), bce, acc

#mode: pre_trained, test
def load_model(model, mode='pre_trained'):
    sd = torch.load(os.path.join(config.save_path, mode + "_" + model.name))
    
    model.load_state_dict(sd['model'])
    model.iter = sd['iter']
    if model.name == 'generator':
        model.optimizer.optimizer.load_state_dict(sd['optimizer'])
        current_ppl = sd['current_ppl']
    else: 
        model.predict_real.optimizer.load_state_dict(sd['predict_real_optimizer'])
    
    current_acc = sd['current_acc']
    current_loss = sd['current_loss']

    if model.name == 'generator':
        return model, current_loss, current_ppl, current_acc
    else:
        return model, current_loss, current_acc
